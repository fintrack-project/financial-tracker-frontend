...
const MarketAverageData: React.FC<MarketAverageDataProps> = ({ indexName, marketData }) => {
  ...
  return (
    <div className="market-item">
      <h2>{indexName}</h2>
      <p>Index: {parseFloat(marketData.price).toFixed(2)}</p>
      <p>
        Change: <span className={`change ${parseFloat(marketData.price_change) >= 0 ? 'positive' : 'negative'}`}>
          {parseFloat(marketData.price_change).toFixed(2)}
        </span>
      </p>
      <p>
        Change (%): <span className={`change ${parseFloat(marketData.percent_change) >= 0 ? 'positive' : 'negative'}`}>
          {parseFloat(marketData.percent_change).toFixed(2)}%
        </span>
      </p>
      <p>Highest: {parseFloat(marketData.price_high).toFixed(2)}</p>
      <p>Lowest: {parseFloat(marketData.price_low).toFixed(2)}</p>
    </div>
  );
};
